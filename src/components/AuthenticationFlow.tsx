        ...
        <Card className="shadow-medium">
          <CardHeader className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground text-center">
              {!isCredentialsValidated ? 'Ingrese sus credenciales para acceder al sistema' : 'Seleccione el perfil con el que desea trabajar'}
            </p>
          </CardHeader>
          <CardContent>
          ...
          <form onSubmit={handleCredentialsSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="username" type="text" placeholder="Ingrese su usuario" value={credentials.username} onChange={e => setCredentials(prev => ({
                  ...prev,
                  username: e.target.value
                }))} disabled={isLoading} className="h-11" required />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="password">Contraseña</Label>
                    <div className="relative">
                      <Input id="password" type={showPassword ? "text" : "password"} placeholder="Ingrese su contraseña" value={credentials.password} onChange={e => setCredentials(prev => ({
                    ...prev,
                    password: e.target.value
                  }))} disabled={isLoading} className="h-11 pr-10" required />
                      <Button type="button" variant="ghost" size="sm" className="absolute right-0 top-0 h-full px-3 py-2 hover:bg-transparent" onClick={() => setShowPassword(prev => !prev)} disabled={isLoading}>
                ...
                <Button type="submit" className="w-full h-11 bg-gradient-primary hover:bg-gradient-primary/90" disabled={isLoading}>
                  {isLoading ? <div className="flex items-center gap-2">
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                      Validando credenciales...
                    </div> : 'Validar Credenciales'}
                </Button>
              </form> :
          ...
          <form onSubmit={handleProfileSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selectedProfile} onValueChange={setSelectedProfile}>
                    ...
                    return <SelectItem key={profile.id} value={profile.id} className="cursor-pointer hover:bg-muted/50 focus:bg-muted/50 bg-background">
                ...
                <div className="flex gap-3">
                  <Button type="button" variant="outline" className="flex-1 h-11" onClick={() => {
                ...
                setError('');
                  ...
                  <Button type="submit" className="flex-1 h-11 bg-gradient-primary hover:bg-gradient-primary/90" disabled={!selectedProfile}>
                    Ingresar al Sistema
                  </Button>
                </div>
              </form>}
            ...
            {!isCredentialsValidated && <div className="mt-6 p-4 bg-muted/30 rounded-lg">
